...
- An attribute the listing does not state is never reported as "no", as `0`, or as any empty-looking falsy value — a fabricated fact could cost the team a flat. The presentation is settled in `@context/foundation/prd.md` (Open Questions, resolved block); `src/components/offers/OfferParameters.astro` is the reference.
