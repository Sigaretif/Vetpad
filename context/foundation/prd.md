...
- FR-005: System validates the pasted URL. An invalid URL shows an error. The product exists to buy a flat, so only a listing that is a **sale of a flat** is accepted: a rental listing, or a listing for any other property type, is refused with an explicit message naming which of the two it failed on, and nothing is saved. Transaction and property type are knowable only from the fetched listing, so that refusal happens after the fetch and leaves no record behind — it is a refusal, not a partial save. Before the duplicate check the URL is normalised by stripping query parameters, so tracking identifiers do not disguise a known listing as a new one. A URL already present in the workspace redirects the member to the existing card, shows a notice naming the member who already saved it, and offers a manual re-fetch instead of creating a duplicate. Priority: must-have
...
- **No rentals, and no property type other than flats.** The card's parameters and the audit's vocabulary are built around buying a flat — floor, service charge, ownership form. A rental or a house is refused at ingestion rather than stored as a listing most of whose fields read "unknown". The operative rule is in FR-005.
