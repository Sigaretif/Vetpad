...
- [x] 1.7 Job `smoke` i `ci` w GitHub Actions są zielone na PR/pushu tej zmiany — 2e4fc69
...
- [x] 2.1 W README nie ma tras rejestracji: `grep -nE "/auth/signup|/auth/confirm-email" README.md` nic nie zwraca — f521935
- [x] 2.2 CLAUDE.md nie wskazuje usuniętego pliku: `grep -n "signup.ts" CLAUDE.md` nic nie zwraca — f521935
- [x] 2.3 Runbook nie opisuje rejestrującego smoke: `grep -n "registers a fresh account" context/foundation/deployment-runbook.md` nic nie zwraca — f521935
- [x] 2.4 Formatowanie przechodzi: `npm run lint` — f521935

#### Manual

- [x] 2.5 W dashboardzie hostowanego projektu „Allow new users to sign up” jest wyłączone, a `GET $SUPABASE_URL/auth/v1/settings` zwraca `"disable_signup": true` — f521935
- [x] 2.6 Trzy konta zespołu istnieją w hostowanym projekcie i każdy członek zalogował się na `https://vetpad.vetpad.workers.dev` — f521935
- [x] 2.7 Po wdrożeniu fazy 1 na produkcji `/auth/signup` zwraca 404, a kontrole z „Verifying a deploy” dają oczekiwane wyniki — f521935
